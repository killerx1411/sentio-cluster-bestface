...
## Combined pipeline (detect → cluster → best face)

This repo integrates with **`profile-clustering`** (sibling folder at repo root):

| Step | Module | What it does |
|------|--------|----------------|
| 1 | `profile-clustering/face_cluster.py` | Sample video frames, detect faces, build embeddings |
| 2 | `profile-clustering/face_cluster.py` | HDBSCAN + agglomerative merge → person clusters |
| 3 | `Best-Face/face_registration` | Filter, score, temporal dedup, save best face per cluster |

### CLI (from repo root `checkidk/`)

```bash
cd Best-Face/face_registration && pip install -r requirements.txt
cd ../..
python run_face_pipeline.py profile-clustering/input_videos/your_video.mp4
python run_face_pipeline.py your_video.mp4 --known-n 26 --force-redetect
```

Output: `Best-Face/face_registration/database/person_0001/best_face.jpg`, etc.

### HTTP API (full video path)
...
curl -X POST http://localhost:8000/api/v1/register/video \
  ...
  -d '{
    "job_id": "job_001",
    "video_path": "C:/path/to/lecture.mp4",
    "known_n_persons": 26
  }'
```
